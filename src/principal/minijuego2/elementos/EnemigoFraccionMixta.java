...
import principal.minijuego2.fracciones.FraccionMixta;
...
public class EnemigoFraccionMixta extends ObjetoJuego implements EntidadB{
	...
	public EnemigoFraccionMixta(double x, double y, Controlador controlador, GestorPrincipal2 minijuego2, int tipoFraccion) {
		super(x,y);
		...
		switch(imagenAleatoria) {
		 ...
		 f_mixta = new FraccionMixta(numerador, denominador, numEntero);
	...
	public void actualizar() {
		...
		for(int i =0; i<minijuego2.ea.size(); i++) {
			...
			if(Colisiones.Colision(this, tempEnt )  &&  tipoFraccion==2 && imagenAleatoria ==0) {
			...
			} else if(Colisiones.Colision(this, tempEnt )  &&  tipoFraccion==2 && imagenAleatoria ==1) {
				controlador.quitarEntidad(tempEnt);
				controlador.quitarEntidad(this);
				GestorPrincipal2.PUNTAJE +=50;
				minijuego2.puntos.reproducir();
				minijuego2.setEnemigos_disparados(minijuego2.getEnemigos_disparados() +1);
				minijuego2.setEnemigosVivos(minijuego2.getEnemigosVivos() -1);
			}
	...
	private void imprimirFraccion(Graphics g) {
		numerador = f_mixta.getNumerador();
		denominador = f_mixta.getDenominador();
		numEntero = f_mixta.getNumeroEntero();
