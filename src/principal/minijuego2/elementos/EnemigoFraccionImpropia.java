...
import principal.minijuego2.fracciones.FraccionImpropia;
...
public class EnemigoFraccionImpropia extends ObjetoJuego implements EntidadB{
	...
	private FraccionImpropia f_impropia;
	private int numerador, denominador, numEntero;
	private int distanciax1, distanciax2, distanciax3;
	private Animacion anim;
	private HojaSprites hs;
	
	public EnemigoFraccionImpropia(double x, double y, Controlador controlador, GestorPrincipal2 minijuego2, int tipoFraccion) {
		super(x,y);
		...
		switch(imagenAleatoria) {
		 ...
		 f_impropia = new FraccionImpropia(numerador, denominador);
	...
	public void actualizar() {
		...
		for(int i =0; i<minijuego2.ea.size(); i++) {
			...
			if(Colisiones.Colision(this, tempEnt ) &&  tipoFraccion==1 && imagenAleatoria==0) {
			...
			} else if(Colisiones.Colision(this, tempEnt ) &&  tipoFraccion==1 && imagenAleatoria==1) {
				controlador.quitarEntidad(tempEnt);
				controlador.quitarEntidad(this);
	...
	private void imprimirFraccion(Graphics g) {
		numerador = f_impropia.getNumerador();
		denominador = f_impropia.getDenominador();
