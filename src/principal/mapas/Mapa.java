...
public class Mapa {
	...
	private int[] extraerSprites(final String[] cadenasSprites) {
	 ...
	 private void actualizarAreasColision() {
	        ...
	        for (int y = 0; y < this.alto; y++) {
	            for (int x = 0; x < this.ancho; x++) {
	            	//MODIFICAR ELEMENTOS PRINCIPALES
	 ...
	 private void actualizarZonaSalida() {
		 	...
		 	int puntoX = ((int) puntoSalida.getX()) * Constantes.LADO_SPRITE - ElementosPrincipales.jugador.getPosicionXInt() + MARGEN_X;
		 	int puntoY = ((int) puntoSalida.getY()) * Constantes.LADO_SPRITE - ElementosPrincipales.jugador.getPosicionYInt() + MARGEN_Y;
	        ...
	        if (!objetosMapa.isEmpty()) {
	            ...
	            for (int i = 0; i < objetosMapa.size(); i++) {
	                ...
	                final Rectangle posicionContenedor = new Rectangle(
	                        contenedor.getPosicion().x * Constantes.LADO_SPRITE, contenedor.getPosicion().y * Constantes.LADO_SPRITE, 
	        ...
	        for (int y = 0; y < this.alto; y++) {
	         ...
	         if (!objetosMapa.isEmpty()) {
	              for (ContenedorObjetos contenedor : objetosMapa) {
	                   final int puntoX = contenedor.getPosicion().x * Constantes.LADO_SPRITE - ElementosPrincipales.jugador.getPosicionXInt() + MARGEN_X;
	                   final int puntoY = contenedor.getPosicion().y * Constantes.LADO_SPRITE - ElementosPrincipales.jugador.getPosicionYInt()  + MARGEN_Y;
	        ...
	        if (!enemigos.isEmpty()) {
	             for (Enemigo enemigo : enemigos) {
	                 final int puntoX = (int) enemigo.getPosicionX() * Constantes.LADO_SPRITE
	                         - (int) ElementosPrincipales.jugador.getPosicionX() + MARGEN_X;
	                 final int puntoY = (int) enemigo.getPosicionY() * Constantes.LADO_SPRITE
	                         - (int) ElementosPrincipales.jugador.getPosicionY() + MARGEN_Y;
